...
@st.cache_data(ttl=300)
def cargar_induccion():
    """
    Carga el CSV de inducción más reciente desde raw/.
    Busca automáticamente el archivo más nuevo con prefijo 'induccion_'.
    Retorna métricas calculadas dinámicamente.
    """
    import glob
    from datetime import datetime

    RAW = os.path.join(BASE, "raw")
    FECHA_CORTE_BASE = pd.Timestamp("2026-02-14")

    # Buscar el CSV más reciente con prefijo induccion_
    patron = os.path.join(RAW, "induccion_*.csv")
    archivos = sorted(glob.glob(patron), reverse=True)

    if not archivos:
        return None  # fallback a valores hardcodeados

    ruta = archivos[0]  # el más reciente por nombre (formato induccion_DDMMYY.csv)
    ...
    try:
        ...
        df["fecha_parsed"] = pd.to_datetime(
            df["Created Date"], format="%b %d, %Y %I:%M %p", errors="coerce"
        )

        # Filtrar por fecha de corte
        df_f = df[df["fecha_parsed"] >= FECHA_CORTE_BASE].copy()

        if df_f.empty:
            return None

        # Métricas
        n_contactos      = len(df_f)
        n_secciones      = int(df_f["seccion_electoral_text"].dropna().nunique())
        cel_validos      = (
            df_f["celular_encuestado_number"]
            .dropna()
            .astype(str)
        ...
        pct_celular      = round(cel_validos / n_contactos * 100) if n_contactos else 60
        fecha_max        = df_f["fecha_parsed"].max()
        fecha_corte_str  = fecha_max.strftime("%-d %b %Y").lower() if pd.notna(fecha_max) else "—"

        return {
            "n_contactos":      n_contactos,
            "n_secciones":      n_secciones,
            "pct_celular":      pct_celular,
...
def main():
    ...
    # ── Métricas operativas — calculadas dinámicamente desde induccion_*.csv ──
    TAMANO_HOGAR_ITER = 3.81
    FECHA_CORTE_ENCUESTA = "10 feb 2026"

    ind = cargar_induccion()
    if ind:
        N_CONTACTOS           = ind["n_contactos"]
        N_SECCIONES_CUBIERTAS = ind["n_secciones"]
        PCT_CELULAR           = ind["pct_celular"]
        FECHA_CORTE_OPERATIVO = ind["fecha_corte_str"]
    else:
        # Fallback a últimos valores conocidos si no hay CSV disponible
        N_CONTACTOS           = 3_919
